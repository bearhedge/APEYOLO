...
/**
 * Test function to validate Step 3 logic
 */
export async function testStep3(): Promise<void> {
  console.log('Testing Step 3: Strike Selection\n');
  ...
  for (const direction of directions) {
    ...
    const selection = await selectStrikes(direction, underlyingPrice);

    if (selection.putStrike) {
      console.log(`PUT Strike: $${selection.putStrike.strike}`);
      console.log(`  Delta: ${selection.putStrike.delta}`);
      console.log(`  Bid/Ask: $${selection.putStrike.bid}/$${selection.putStrike.ask}`);
    }

    if (selection.callStrike) {
      console.log(`CALL Strike: $${selection.callStrike.strike}`);
      console.log(`  Delta: ${selection.callStrike.delta}`);
      console.log(`  Bid/Ask: $${selection.callStrike.bid}/$${selection.callStrike.ask}`);
    }

    console.log(`\nExpected Premium: $${selection.expectedPremium}`);
    console.log(`Margin Required: $${selection.marginRequired}`);
